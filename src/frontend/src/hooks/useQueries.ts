...
import type { Course, Enrollment, Testimonial, UserProfile } from "../backend";
...
export function useUpdateEnrollmentProgress() {
  ...
  return useMutation({
    mutationFn: async ({
      enrollmentId,
      progress,
    }: { enrollmentId: bigint; progress: bigint }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useSubmitTestimonial() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(
        "Testimonial submitted! It will appear after admin approval.",
      );
    },
    onError: () => {
...
export function useAssignUserRole() {
  ...
  return useMutation({
    mutationFn: async ({
      userId,
      role,
    }: { userId: string; role: import("../backend").UserRole }) => {
      if (!actor) throw new Error("Actor not available");
