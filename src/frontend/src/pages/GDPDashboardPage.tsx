...
const impactAreas = [
  ...
  },
  {
    ...
    description:
      "Sustainable bamboo farming for construction and export markets.",
    icon: "🎋",
  ...
  },
  {
    title: "Plant Nursery",
    description:
      "High-quality seedling production for reforestation and agriculture.",
    icon: "🌱",
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-primary/80 to-primary/40 flex items-center">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 w-full">
            ...
            <p className="text-white/80 text-sm sm:text-base max-w-xl">
              Tracking our contribution to Bhutan's 10x GDP growth goal through
            ...
            <Card className="border-blue-500/20">
              <CardContent className="pt-6 pb-6">
                ...
                <p className="text-3xl font-bold text-foreground">
                  {isError
                    ? "200"
                    : Number(displayStats.trainedFarmers).toLocaleString()}
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  And growing
                </p>
            ...
            <Card className="border-amber-500/20">
              <CardContent className="pt-6 pb-6">
                ...
                <p className="text-3xl font-bold text-foreground">
                  ...
                  {isError
                    ? "1.5M"
                    : `${(displayStats.revenueGenerated / 1_000_000).toFixed(1)}M`}
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  Total farmer income
                </p>
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-sm text-muted-foreground">
                ...
                <span className="font-semibold text-foreground">
                  {isError ? "15" : displayStats.gdpProgress.toFixed(1)}% of
                  100%
                </span>
              </div>
              <Progress
              ...
              <p className="text-xs text-muted-foreground">
                Target: Achieve 10x GDP growth through sustainable agricultural
                development by 2034
              </p>
            </div>
          </CardContent>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {milestones.map((milestone) => {
              const trainedFarmers = isError
                ? 200
                : Number(displayStats.trainedFarmers);
              const gdpProg = isError ? 15 : displayStats.gdpProgress;
              const isAchieved = milestone.label.includes("Farmers")
                ...
                >
                  <CardContent className="pt-4 pb-4 flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p
                        className={`text-sm font-medium ${isAchieved ? "text-green-700 dark:text-green-400" : "text-foreground"}`}
                      >
                        {milestone.label}
                      </p>
                      {isAchieved && (
