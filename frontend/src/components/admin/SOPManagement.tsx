...
import { useSOPs, useCreateSOP, useUpdateSOP, useDeleteSOP } from '../../hooks/useQueries';
...
export default function SOPManagement() {
  ...
  const handleEdit = (sop: SOP) => {
    ...
    setFormData({
      ...
      qualityStandards: sop.qualityStandards.length > 0 ? sop.qualityStandards : [''],
      documentationRequirements: sop.documentationRequirements.length > 0 ? sop.documentationRequirements : [''],
    });
    setOpen(true);
  };
  ...
  const addArrayItem = (field: 'procedures' | 'roles' | 'qualityStandards' | 'documentationRequirements') => {
    setFormData({ ...formData, [field]: [...formData[field], ''] });
  };

  const removeArrayItem = (field: 'procedures' | 'roles' | 'qualityStandards' | 'documentationRequirements', index: number) => {
    const newArray = formData[field].filter((_, i) => i !== index);
    setFormData({ ...formData, [field]: newArray.length > 0 ? newArray : [''] });
  };

  const updateArrayItem = (field: 'procedures' | 'roles' | 'qualityStandards' | 'documentationRequirements', index: number, value: string) => {
    ...
    try {
      if (editingSOP) {
        await updateSOP.mutateAsync({
          ...
          documentationRequirements: filterEmpty(formData.documentationRequirements),
        });
      ...
      toast.error(error.message || `Failed to ${editingSOP ? 'update' : 'create'} SOP`);
    }
  };
              ...
              <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {formData.procedures.map((procedure, index) => (
                      <div key={index} className="flex gap-2">
                        <Input
                          value={procedure}
                          onChange={(e) => updateArrayItem('procedures', index, e.target.value)}
                  ...
                  <div className="space-y-2">
                    ...
                    {formData.qualityStandards.map((standard, index) => (
                      <div key={index} className="flex gap-2">
                        <Input
                          value={standard}
                          onChange={(e) => updateArrayItem('qualityStandards', index, e.target.value)}
                  ...
                  <div className="space-y-2">
                    ...
                    {formData.documentationRequirements.map((requirement, index) => (
                      <div key={index} className="flex gap-2">
                        <Input
                          value={requirement}
                          onChange={(e) => updateArrayItem('documentationRequirements', index, e.target.value)}
                        ...
                        {formData.documentationRequirements.length > 1 && (
                          <Button
                            ...
                            variant="outline"
                            size="icon"
                            onClick={() => removeArrayItem('documentationRequirements', index)}
                          >
                            <X className="h-4 w-4" />
                          </Button>
                        )}
                  ...
                  <Button type="submit" className="w-full" disabled={createSOP.isPending || updateSOP.isPending}>
                    {createSOP.isPending || updateSOP.isPending ? 'Saving...' : editingSOP ? 'Update SOP' : 'Create SOP'}
                  </Button>
                </form>
              </DialogContent>
            </Dialog>
              ...
              <TableBody>
                {sops.map((sop) => (
                  <TableRow key={Number(sop.id)}>
                    ...
                    <TableCell className="max-w-md truncate">{sop.description}</TableCell>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDeleteConfirm} disabled={deleteSOP.isPending}>
