...
import { FileText, CheckCircle2, Users, ClipboardCheck, FileCheck } from 'lucide-react';
import AccessDeniedScreen from '../components/AccessDeniedScreen';
import { useInternetIdentity } from '../hooks/useInternetIdentity';

export default function SOPsPage() {
  const { identity } = useInternetIdentity();
  const { data: hydroponicSOPs, isLoading: hydroponicLoading } = useSOPsByProductionType('hydroponic');
  const { data: nurserySOPs, isLoading: nurseryLoading } = useSOPsByProductionType('nursery');
  const { data: bambooSOPs, isLoading: bambooLoading } = useSOPsByProductionType('bamboo');

  if (!identity) {
    return <AccessDeniedScreen message="Please login to access Standard Operating Procedures" />;
  }
  ...
  const renderSOPs = (sops: any[] | undefined, isLoading: boolean, productionType: string) => {
    if (isLoading) {
      return (
                  ...
                  <AccordionItem value="procedures">
                    ...
                    <AccordionContent>
                      <ol className="list-decimal list-inside space-y-2 text-sm">
                        {sop.procedures.map((procedure: string, index: number) => (
                          <li key={index} className="text-muted-foreground">{procedure}</li>
                        ))}
                      </ol>
                    </AccordionContent>
                  </AccordionItem>
                  ...
                  <AccordionItem value="roles">
                    <AccordionTrigger>
                      <div className="flex items-center gap-2">
                        ...
                        <span>Roles and Responsibilities ({sop.roles.length})</span>
                      </div>
                    </AccordionTrigger>
                    <AccordionContent>
                  ...
                  <AccordionItem value="quality">
                    <AccordionTrigger>
                      <div className="flex items-center gap-2">
                        ...
                        <span>Quality Standards ({sop.qualityStandards.length})</span>
                      </div>
                    </AccordionTrigger>
                    <AccordionContent>
                      <ul className="list-disc list-inside space-y-2 text-sm">
                        {sop.qualityStandards.map((standard: string, index: number) => (
                          <li key={index} className="text-muted-foreground">{standard}</li>
                        ))}
                      </ul>
                    </AccordionContent>
                  </AccordionItem>
                  ...
                  <AccordionItem value="documentation">
                    <AccordionTrigger>
                      <div className="flex items-center gap-2">
                        ...
                        <span>Documentation Requirements ({sop.documentationRequirements.length})</span>
                      </div>
                    </AccordionTrigger>
                    <AccordionContent>
                      <ul className="list-disc list-inside space-y-2 text-sm">
                        {sop.documentationRequirements.map((requirement: string, index: number) => (
    ...
    <div className="container py-12">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Access detailed procedures, quality standards, and documentation requirements for all production areas.
        </p>
      </div>
